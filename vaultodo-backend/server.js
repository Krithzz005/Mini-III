...
const isAdmin = (req, res, next) => {
  ...
  else res.status(403).send({ error: 'Access denied: Admin privileges required.' });
};
...
app.post('/api/auth/forgot-password', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email });
    if (!user) return res.status(200).send({ message: 'If an account with that email exists, a password reset link has been sent.' });
...
app.post('/api/tasks', auth, async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.user._id)) throw new Error('Invalid user ID format.');
...
app.patch('/api/tasks/:id', auth, async (req, res) => {
  try {
    const task = await Task.findOneAndUpdate({ _id: req.params.id, user_id: req.user._id }, req.body, { new: true, runValidators: true });
...
app.patch('/api/admin/users/:userId/role', auth, isAdmin, async (req, res) => {
  try {
    ...
    if (!['user', 'admin'].includes(newRole)) return res.status(400).send({ error: 'Invalid role specified.' });
